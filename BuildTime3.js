...
function search(loc, pos) {
  let result = [];
  loc = loc.toLowerCase();
  pos = pos.toLowerCase();

  for (let i = 0; i < jobs.length; i++) {
    if ((jobs[i].location.toLowerCase().includes(loc)) && (jobs[i].title.toLowerCase().includes(pos))) {
      result.push(jobs[i]);
  ...
  console.log('Risultati di ricerca: ' + count);

  return result;
}

console.log(search('us', 'dev'));
